...
<?php
25.5.3 Виды авторизации (01:53:35)
    1) через адресную строку
    2) через куки
В любом случае нам нужно делать отметку, что пользователь зарегистрировался.
...
В куках хранится не авторизация - в куках хранится идентификатор (ID сессии авторизации).

Для проверки, что у нас хранится в куках создадим в корме файл temp.php:
echo '<pre>';
print_r($_COOKIE);
echo '</pre>';
...
25.6.3 Блокировка пользователя
Делается просто - высталяем в базе в строчке `active` = 2

26.1 allpages.php (02:15:15)
...
26.2 Проверка на авторизацию в скинах (*.tpl):
if(isset($_SESSION['user'])) {
    //форма авторизации
} else {
...
27. Домашнее задание: (02:18:55)
27.1. Сделать авто-авторизацию на сайте. То есть для тех, у кого стоит при обычной авторизации галочка, что необходимо
...
Что такое автоматически авторизировать? Это значит, что мы должны создать переменную пользователя $_SESSION['user'] в БД
$_SESSION['user'] = mysqli_fetch_assoc($res);
...
Если хакер сможет своровать ID и хеш, то он сможет залониться и поэтому лучший вариант и в БД и в куках хранить бонусом
для сравнения: 1-й вариант - IP, 2-й вариант - браузер $_SERVER['HTTP_USER_AGENT'].
Но с IP есть проблема - он у большинства динамический и автоматическая авторизация не будет работать если чесловек
разлогинился. А HTTP_USER_AGENT можно подделать


27.2. Комментарии только для авторизированных пользователей сделать (улучшить модуль комментирования на сайте).
27.3. Защитить модуль news от доступа пользователей, у которых нет прав смотреть данный раздел. Права есть в таблице
